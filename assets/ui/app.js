...
console.log("video url", vw);

var player = new JSMpeg.Player(vw, { canvas: canvas });
...
var dataSocket = new ReconnectingWebSocket(dw, null, wsOptions);
...
var chart = new SmoothieChart({
    responsive: responsiveSmoothie,
    millisPerPixel: 10,
    grid: { fillStyle: "#ffffff" },
    interpolation: "linear",
    maxValue: 135,
    minValue: -135,
    labels: { fillStyle: "#0024ff", precision: 0 },
  }), //interpolation:'linear
...
dataSocket.onmessage = function (event) {
  try {
    ...
    if (wrapEncoder) {
      //wrap and convert to degrees
      enc =
        (Math.atan2(
          Math.sin((obj.enc / (encoderPPR / 2)) * Math.PI),
          Math.cos((obj.enc / (encoderPPR / 2)) * Math.PI)
        ) /
          Math.PI) *
        180;
    ...
    if (!isNaN(thisTime) && !isNaN(enc)) {
      ...
      if (debug) {
        console.log(
          "NaN so not logging to smoothie",
          delay,
          thisDelay,
          msgTime,
          enc
